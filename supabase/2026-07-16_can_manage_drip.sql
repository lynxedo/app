-- can_manage_drip permission for the Drip Marketing builder (Increment 2).
-- Additive: new user_profiles flag + widen get_admin_users() to return it (so the
-- Admin → People toggle grid can read/show it). Applied to the shared DB 2026-07-16.
--
-- ⚠ get_admin_users() is SECURITY DEFINER; adding a return column forces DROP+CREATE,
-- which re-grants EXECUTE to anon/PUBLIC by Supabase default. We re-REVOKE and restore
-- the exact prior ACL (authenticated + service_role only). Same lesson as the
-- integrations/beta/people widenings.

alter table public.user_profiles
  add column if not exists can_manage_drip boolean not null default false;
