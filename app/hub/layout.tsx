...
export default async function HubLayout({ children }: { children: React.ReactNode }) {
  ...
  const [memberRoomsResult, hubUsersResult, meResult, profileResult, announcementsResult, myPresenceResult] = await Promise.all([
    ...
    supabase.from('user_profiles').select('role, hub_text_size, hub_pinned_ids, can_access_tracker, can_access_call_log, can_access_lawn, can_access_zone_sizer, can_access_timesheet, can_access_routing, can_access_books, can_access_fleet, can_access_dialer, can_access_marketing, can_admin_marketing, can_access_forms, can_admin_forms, can_access_daily_log_v2, dialer_global_ring, can_admin_people, can_admin_hub, can_admin_routing, can_admin_timesheet, can_admin_fleet, can_admin_daily_log, can_admin_zone_sizer, can_admin_dialer, rail_config').eq('id', user.id).single(),
