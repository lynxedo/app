...
export default async function HubAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: profile } = await supabase
    .from('user_profiles')
    .select('role, can_admin_people, can_admin_hub, can_admin_guardian, can_admin_ai, can_admin_txt, can_admin_announcements, can_admin_file_tags, can_admin_routing, can_admin_timesheet, can_admin_fleet, can_admin_daily_log, can_admin_dialer, can_admin_contacts, can_admin_products, can_admin_integrations')
    .eq('id', user.id)
    .single()
