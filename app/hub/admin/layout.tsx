...
export default async function HubAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: profile } = await supabase
    .from('user_profiles')
    .select('role, can_admin_people, can_admin_hub, can_admin_routing, can_admin_timesheet, can_admin_fleet, can_admin_daily_log, can_admin_zone_sizer, can_admin_dialer')
    .eq('id', user.id)
    .single()
