...
export default async function SettingsPage() {
  ...
  const initialTxtDndEnabled = profileResult.data?.txt_dnd_enabled ?? false
  const initialTxtDndSchedule = (profileResult.data?.txt_dnd_schedule ?? null) as Record<string, unknown> | null
  const initialInboxDndEnabled = profileResult.data?.inbox_dnd_enabled ?? false
  const initialInboxDndSchedule = (profileResult.data?.inbox_dnd_schedule ?? null) as Record<string, unknown> | null
    ...
    <div className="flex-1 min-h-0 overflow-y-auto bg-gray-950 text-white">
      ...
      <main className="max-w-2xl mx-auto px-4 md:px-6 py-6 md:py-10">
        ...
        <SettingsForm
          ...
          initialTabsEnabled={profileResult.data?.hub_tabs_enabled ?? true}
          notifPref={notifPref}
          ...
          initialTxtDndEnabled={initialTxtDndEnabled}
          initialTxtDndSchedule={initialTxtDndSchedule}
          initialInboxDndEnabled={initialInboxDndEnabled}
          initialInboxDndSchedule={initialInboxDndSchedule}
