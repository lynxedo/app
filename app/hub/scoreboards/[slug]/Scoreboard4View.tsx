...
function Card({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl border border-amber-400/12 bg-gradient-to-br from-[#0f2e47] to-[#1a3d5c] p-5 ${className}`}>
...
function Dashboard({ data, meta }: { data: Payload; meta: ScoreboardMeta }) {
  ...
  return (
    <div className="mx-auto max-w-[1280px] px-4 md:px-6 pb-12 pt-2">
      ...
      {data.techs.length === 0
        ? <Card><div className="py-8 text-center text-sm text-slate-500">No PW technicians assigned. Add them in Admin → Scoreboards.</div></Card>
        : data.techs.map(tech => (
...
export default function Scoreboard4View({ meta }: { meta: ScoreboardMeta }) {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto bg-[#0b1929] text-slate-200">
      <header className="flex items-center gap-3.5 border-b border-amber-400/15 bg-gradient-to-br from-[#0f2e47] to-[#1a3d5c] px-5 py-4 max-md:pl-14">
