...
function MarketingTab() {
  return (
    <>
      ...
      <Section title="Drip (speed-to-lead texting)">
        <p>Open <strong className="text-white">Marketing → Drip</strong> to text new leads the moment they arrive and follow up automatically. The idea: whoever answers a lead first usually wins it — so instead of waiting for someone to notice a new lead, Drip sends a friendly first text within a couple minutes, then a few more over the next days if they don&apos;t respond. The instant the lead texts back, the sequence <strong className="text-white">stops automatically</strong> so a real person (or Amber) can take over.</p>
        <p className="mt-2"><strong className="text-white">Build a campaign.</strong> Hit <em>+ New campaign</em>, give it a name, and pick a <strong className="text-white">trigger</strong> — <em>Any new lead</em> (every new lead, from any source), <em>A specific lead source</em> (e.g. only <em>Angi Lead</em> — type the source exactly as it appears on the lead), or <em>Manual</em>. Then add your <strong className="text-white">texts</strong>: the first one <em>sends immediately</em>; for each follow-up, set how long to <em>wait</em> (hours or days) before it goes out. Use <code className="text-gray-300">{'{{first_name}}'}</code> to personalize. A classic sequence is: <em>send now</em> → <em>wait 1 hour</em> → <em>wait 1 day</em> → <em>wait 3 days</em>.</p>
        <p className="mt-2"><strong className="text-white">Draft → Activate.</strong> New campaigns save as a <strong className="text-white">draft</strong> and do nothing until you <strong className="text-white">Activate</strong> them. Activating only enrolls leads that arrive <em>after</em> that moment — it won&apos;t text your existing leads. <strong className="text-white">Pause</strong> freezes everyone in place; <strong className="text-white">Monitor</strong> shows who&apos;s enrolled, which text they&apos;re on, and how many have replied or finished.</p>
        <p className="mt-2"><strong className="text-white">Replies stop the drip.</strong> As soon as a lead texts (or calls) back, their sequence is paused and the conversation shows up in the normal Txt inbox for a human to handle. Texting <em>STOP</em> opts them out entirely. Every first text automatically includes the business name and a &ldquo;Reply STOP to opt out&rdquo; line, and nothing sends outside <strong className="text-white">quiet hours</strong>.</p>
        <p className="mt-2"><strong className="text-white">Settings.</strong> The <em>Settings</em> button sets the defaults for all campaigns: <strong className="text-white">who texts are sent as</strong> (drip texts land in that team member&apos;s Txt inbox, so replies surface there — this must be set before a campaign can send), <strong className="text-white">quiet hours</strong> (default 8am–8pm, texts due outside the window wait until it opens), a <strong className="text-white">max texts per lead per day</strong> cap, and your <strong className="text-white">business name</strong>.</p>
