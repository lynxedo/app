  ...
  return (
    <>
      ...
      <Section title="Assigning &amp; collaborating">
        <p>Each conversation has an <strong className="text-white">owner</strong> (the chip reads &quot;Owner: You&quot; or the teammate&apos;s name) plus optional <strong className="text-white">members</strong> who can also see and reply. An <strong className="text-white">unclaimed</strong> thread has no owner — anyone with Txt2 can <strong className="text-white">Claim</strong> it to become the owner. A thread <em>owned by someone else</em> you can <strong className="text-white">Join</strong> yourself (one tap) so you never have to wait to be added. To pull <em>someone else</em> in, the owner or a Texting Manager uses <strong className="text-white">+ member</strong> (they get a push so they know they&apos;ve been added). A member can <strong className="text-white">Leave</strong> a thread anytime. <strong className="text-white">Archiving</strong> is owner-level — only the conversation&apos;s owner or a Texting Manager can archive or reopen it.</p>
      ...
      <Section title="Unified inbox (calls + voicemails)">
        <p>Every customer thread is one chronological story of <em>every</em> way you&apos;ve talked: texts as bubbles, plus inline markers for <strong className="text-white">📞 calls</strong> and <strong className="text-white">🎙 voicemails</strong> — visible to <strong className="text-white">anyone with Txt2</strong> (you already see the texts; now the calls and voicemails sit right alongside them). Tap a call or voicemail marker to expand it — play the recording right in the app, read the AI summary and sentiment, and open the full transcript. A missed call that left a voicemail shows as <em>one</em> combined marker, not two.</p>
      ...
      <AdminOnly>
        ...
        <p><strong className="text-white">Two access levels.</strong> Everyone with Txt2 shares the inbox — they can <strong className="text-white">read</strong> any conversation in <strong className="text-white">All</strong>, start new ones, take notes, and send group messages. <strong className="text-white">Sending a reply is limited to the conversation&apos;s owner and its members</strong>: an unclaimed thread shows <strong className="text-white">Claim it</strong> (you become owner); a thread owned by someone else shows <strong className="text-white">Join to reply</strong> (you add yourself as a member). Either way the composer — plus AI suggestions, scheduling, etc. — appears only after that explicit action. <strong className="text-white">Archiving</strong> a thread is owner-or-manager only. <strong className="text-white">Texting Managers</strong> additionally see the unassigned <strong className="text-white">Queue</strong> (which now also holds Guardian auto-text threads), can add or remove <em>other</em> people on a thread, can reassign/archive any thread, and can send <strong className="text-white">Broadcasts</strong>.</p>
  ...
  return (
    <>
      ...
      <Section title="Email Marketing (new)">
        ...
        <p className="mt-2"><strong className="text-white">Campaigns (one-off blasts).</strong> A campaign pairs a <strong className="text-white">template</strong> with a <strong className="text-white">segment</strong> and sends it out. In the Campaigns tab, hit <em>+ New campaign</em>, pick the template, pick who it goes to (a saved segment, or <em>Everyone</em>), and you&apos;ll see a live <strong className="text-white">≈ N recipients</strong> count. Choose <strong className="text-white">Send now</strong> or <strong className="text-white">Schedule</strong> a date/time. Sends go out steadily in the background (throttled, never all at once), and the campaign row shows a live progress bar with <em>sent / failed / skipped</em> counts. Every email automatically carries a one-click unsubscribe and your mailing address (CAN-SPAM), and anyone unsubscribed or suppressed is skipped — even if they opt out <em>after</em> the campaign is queued. Hit <strong className="text-white">Stop</strong> to halt a campaign that&apos;s still sending (remaining recipients are skipped), or <strong className="text-white">Remove</strong> to clear a finished one from the list.</p>
        ...
        <p className="mt-2"><strong className="text-white">Sending identity.</strong> Each company sets its own From address, Reply-To, and sending domain in <strong className="text-white">Admin → Email Marketing</strong>. For emails to land in inboxes, the sending domain has to be verified in Resend (SPF/DKIM/DMARC DNS records) — the admin panel has a setup checklist and a <em>Refresh domain status</em> button. Until the domain shows <em>verified</em>, sends won&apos;t deliver.</p>
        <p className="mt-2"><strong className="text-white">Test it.</strong> Once the domain is verified and the server key is set, use <em>Send a test email to myself</em> on the Email page to confirm everything is wired up.</p>
        ...
        <AdminOnly>Access is per-user via <strong className="text-white">Admin → People → Tools → Email Marketing</strong>; managing the sending identity, contacts, and imports is the separate <em>Email Marketing</em> admin grant.</AdminOnly>
      </Section>
    </>
  )
}
