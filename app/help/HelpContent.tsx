...
<p className="mt-3"><strong className="text-white">If a line shows $0, its programs are missing a visits-per-year.</strong> A book&apos;s annual value is visits-per-year × price, so a program without that number adds nothing. The card tells you how many jobs it couldn&apos;t price and which lines they&apos;re on, rather than quietly reporting zero. Ask an admin to set it — your recurring program list (which programs exist, which are add-ons, and how many visits a year each one runs) is currently maintained directly in the database rather than on a settings screen.</p>
      </Section>
