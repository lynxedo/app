  ...
  return (
    <>
      ...
      <Section title="Daily Log v2 (preview)">
        ...
        <AdminOnly>
          <p className="mt-4"><strong className="text-white">What triggers a record</strong> is the <strong className="text-white">Service Mapping → Line-Item Map</strong> (<strong>Admin → Service Mapping</strong>). Map a Jobber line item (case-insensitive contains or exact) to one or more <strong className="text-white">products</strong>; the chemical details on the record — name, EPA registration #, active ingredient, application rate, batch # — are pulled live from each mapped product in the Products catalog (no re-typing). When a visit completes, every line item is checked against every active mapping; matches roll into the record&apos;s <em>chemicals_applied</em> list (one entry per matched product). Set a product&apos;s mapping inactive to stop it triggering new records — existing records stay intact. <em>(The old per-line-item pesticide mapping screen under Daily Log Admin is retired — mappings now live in Service Mapping, the single source the route-capacity tool also reads.)</em></p>
        </AdminOnly>
      </Section>

      <Section title="Pesticide Records">
        <p>Available at <Link href="/hub/pesticide-records" className="text-sky-400 hover:underline">/hub/pesticide-records</Link>. Auto-generated TDA-compliance records for any pesticide application — captured whether the visit was completed in Daily Log v2 or directly in Jobber. Filter by date range or search by customer / address / technician. Each row shows the customer, application time, applicator, chemicals applied (with EPA numbers), and the weather snapshot at completion. Tap a row for the full detail view.</p>
        ...
        <p>Records are created automatically — you don&apos;t add them by hand. They appear when a visit with matching line items is marked complete, either in Daily Log v2 or in Jobber. If the mapping configuration changes after a record is created, the existing record stays as it was at the time of application (the chemicals_applied list is a snapshot, not a live join).</p>
