...
import { gateProducts, reqStr, strOrNull, numOrNull, isErr } from '@/lib/products-server'
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  for (const [key, max] of [['description', 2000], ['unit', 40], ['epa_reg_number', 100], ['active_ingredient', 200], ['notes', 2000], ['batch_number', 100]] as const) {
    if (key in body) {
  ...
  for (const key of ['package_price', 'package_size'] as const) {
    if (key in body) {
...
// DELETE — remove an item (cascades its sub-items + inventory rows).
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const admin = createAdminClient()
  const { error } = await admin.from('products').delete().eq('id', id).eq('company_id', ctx.companyId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
