...
// PATCH — rename / reorder / activate a group.
...
// DELETE — remove a group. Items in it keep existing (their category_id is set to null).
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const admin = createAdminClient()
  const { error } = await admin.from('product_categories').delete().eq('id', id).eq('company_id', ctx.companyId)
