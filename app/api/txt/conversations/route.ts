...
export async function GET(request: Request) {
  ...
  if (scope === 'search') {
    ...
    let foundQuery = supabase
      .from('txt_conversations')
      .select(
        `id, kind, status, source, assigned_to, archived_by, phone_number_id, last_message_at, last_inbound_at, last_message_preview, last_message_direction, lsa_relay, lsa_location, lsa_service, created_at,
