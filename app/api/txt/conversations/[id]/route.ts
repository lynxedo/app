...
export async function GET(
  ...
  let msgQuery = supabase
    .from('txt_messages')
    .select(
      'id, direction, body, media_urls, status, error_message, twilio_sid, created_at, sent_by, contact_id, phone_number_id, rerouted, sender:hub_users!sent_by ( id, display_name ), number:txt_phone_numbers!txt_messages_phone_number_id_fkey ( label, twilio_number )'
    )
      ...
      supabase
        ...
        .select(
          `id, kind, status, assigned_to, last_message_at, last_inbound_at, created_at, phone_number_id,
           contact:txt_contacts!txt_conversations_contact_id_fkey ( id, name, phone, email, do_not_text, jobber_client_id, notes, in_directory ),
