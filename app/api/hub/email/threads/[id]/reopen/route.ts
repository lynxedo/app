...
// POST /api/hub/email/threads/[id]/reopen — bring a closed thread back into the
// active queue. Status returns to 'assigned' if it still has an assignee, else 'open'.
export async function POST(
  ...
  const { error: updErr } = await admin
    .from('inbox_threads')
    .update({
      status: thread.assigned_to_user_id ? 'assigned' : 'open',
  ...
  after(async () => {
    try {
      ...
      console.warn('[inbox:reopen] broadcast failed', err)
    }
  })
